...
def discount_rewards(r):
    ...
    for i in range(len(r)):
    	if r[-i-1] != 0:
    		curr = 0
    	curr = r[-i-1] + GAMMA*curr
    	discounted_r.append(curr)
    discounted_r = np.array(discounted_r[::-1])
...
def compute_cost(readout,action_holder,reward_holder):
	...
	Returns:
		loss
	""" 
	entropy = tf.multiply(action_holder, tf.log(readout)) + tf.multiply(1 - action_holder, tf.log(1 - readout))
	loss = -tf.reduce_sum( tf.multiply( tf.expand_dims(reward_holder,1), entropy ) )
	return loss
...
def get_action_index(readout_t,epsilon,t):
	...
	"""
	if t < OBSERVE or np.random.rand() <= epsilon:
...
def run_selected_action(a_t,s_t,game_state):
	...
	"""
	frame_next, r_t, terminal = game_state.frame_step(a_t)
	frame_next = cv2.cvtColor(cv2.resize(frame_next, (80, 80)), cv2.COLOR_BGR2GRAY)
	_, frame_next = cv2.threshold(frame_next,1,255,cv2.THRESH_BINARY)
	# s_t = np.stack((x_t, x_t, x_t, x_t), axis = 2)
	s_t1 = np.concatenate((s_t[:,:,1:], np.expand_dims(frame_next,axis=2)), axis=2)
