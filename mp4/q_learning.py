...
def compute_cost(target_q,a_t,q_value):
	...
	q_action = tf.reduce_sum(tf.multiply(a_t, q_value), axis=1)
	# TO DO: Q-Learning Cost.
	cost = tf.reduce_sum(tf.square(q_action-target_q))
	return cost
...
def compute_target_q(next_state_batch,r_batch,readout_j1_batch,minibatch):
	...
	for i in range(len(minibatch)):
		if minibatch[i][4] == True:
			...
			continue
		target_q_batch.append(r_batch[i] + GAMMA*np.amax(readout_j1_batch[i]))
